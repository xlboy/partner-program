import { getRepository, Like, MongoRepository, Repository } from "typeorm";
...
export default class PlanGroupService {
  ...
  async create(
    ...
    try {
      ...
      planGroup.groupNum = await (async () => {
        const allPlanGroupNum = (await this.findAll()).map(
          (item) => item.groupNum
        );
        let groupNum: number;
        while (true) {
  ...
  async verifGroupExist(
    planGroup: Pick<PlanGroup, "groupNum">
  ): Promise<boolean> {
  ...
  async findUserGroupList(
    userId: number
  ): Promise<{ myGroup: PlanGroup[]; otherGroup: PlanGroup[] }> {
